...
boot_alloc(uint32_t n)
{
	...
	uint32_t new_alloc = ROUNDUP(n, PGSIZE);
...
page_init(void)
{
	...
	size_t i;
	size_t first_free_byte = PADDR(boot_alloc(0));

	for (i = 1; i < npages; i++) 
	{
		// i = IOPHYSMEM/PGSIZE,i < EXTPHYSMEM/PGSIZE
		if (i >= PGNUM(IOPHYSMEM) && i < PGNUM(first_free_byte))
		{
			pages[i].pp_ref = 1;
		}
		else {
...
{
	// Fill this function 
	struct PageInfo *physicalPage = page_free_list;
	if (!physicalPage)
	{
		return NULL;
	}
	...
	physicalPage->pp_link = NULL;

    if (alloc_flags & ALLOC_ZERO)
    {
        memset(page2kva(physicalPage), 0, PGSIZE);
    }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
 ...
 //        page_table = (pte_t *)page2kva(pp);
 //        pp->pp_ref++;
 //        *pde = PADDR(page_table) | PTE_P | PTE_W | PTE_U;
 //    }   
